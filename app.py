...
RULES = [
    ...
    {"title": "Aprendizaje continuo", "text": "Sigue en continuo aprendizaje y recuerda enseñar lo que sabes a los demás.",
     "quiz_text": "Sigue en continuo aprendizaje y recuerda enseñar lo que sabes a los demás."},
...
with tab1:
    ...
    if st.session_state.flipped:
        st.markdown(
            f'<div class="card"><p>{idx + 1}. {rule["text"]}</p></div>',
            unsafe_allow_html=True,
        )
    else:
    ...
    def build_quiz():
        ...
        for i in rule_indices:
            ...
            rule_questions.append(
                {"q": RULES[i]["quiz_text"], "correct": RULES[i]["title"], "wrongs": wrongs}
            )
